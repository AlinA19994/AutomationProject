...
from page_objects.web_object.Ad_page.Fourth_page import next_month as nextMonth
from page_objects.web_object.Ad_page.Fourth_page import popup_data
...
class UiActions:
    ...
    @staticmethod
    @allure.step('select date from data picker')
    def data_picker(icon: WebElement, month: WebElement, expect_month, next_month: WebElement, date_popup,
                    day):
        icon.click()  # first click on calendar row
        try:
            while month.text != expect_month:  # Search the month)
                wait(For.ELEMENT_TO_BE_CLICKABLE, nextMonth)
                next_month.click()
                wait(For.ELEMENT_TO_BE_CLICKABLE,popup_data)
        except StaleElementReferenceException:
            wait(For.ELEMENT_TO_BE_CLICKABLE,popup_data)
        cells = date_popup.find_elements(By.TAG_NAME, 'span')  # search all cells
        for cell in cells:  # run on cells
            if cell.text == day:
                cell.click()
                break
